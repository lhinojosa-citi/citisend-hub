...
let projects = [];
let contacts = [];
const DEFAULT_PROJECT_MANUALS = ["manual-instalacion-citisend", "manual-mantenimiento-correctivo", "manual-mantenimiento-preventivo", "tarjetas-citisend", "configuracion-dispositivos"];
const ACCESS_LOGIN_PAGE = 'index.html';
const ADMIN_HUB_PAGE = 'hub.html';
...
const fallbackManuals = [
  {
    ...
    "titulo": "Manual de Gestión de Proyectos",
    "categoria": "Planner",
    "icono": "📋",
    "tipo": "HTML",
    "version": "v1.4",
    "proyecto": "General / Granada",
    "actualizado": "27/06/2026",
    "tiempo": "8 min",
    "autor": "Project Management",
    "descripcion": "Guía interna para gestionar proyectos con Planner, Teams y seguimiento por fases.",
    "archivo": "manuales/gestion-proyectos.html",
    "destacado": true,
    "keywords": "manual gestion proyectos planner teams fases seguimiento project management implantacion granada fcc proyecto"
  },
  {
    "id": "configuracion-dispositivos",
    "titulo": "Manual de Configuración de Dispositivos",
    "categoria": "Software",
    "icono": "📱",
    "tipo": "HTML",
    "version": "v1.4",
    ...
    "tiempo": "10 min",
    "autor": "Operaciones",
    "descripcion": "Procedimiento de configuración, Bluetooth, aplicación Citiservice y validación de dispositivos.",
    "archivo": "manuales/configuracion-dispositivos.html",
    "destacado": true,
    "keywords": "manual configuracion dispositivos citiservice bluetooth instalacion app dispositivos campo granada fcc proyecto"
  },
  {
    "id": "sensor-volumetrico-sloc",
    "titulo": "Manual Instalación Sensor Volumétrico SLOC",
    ...
    "tiempo": "6 min",
    "autor": "Operaciones",
    "descripcion": "Guía resumida para instalar, conectar y validar el sensor volumétrico SLOC, con vídeo de apoyo integrado.",
    "archivo": "manuales/sensor-volumetrico-sloc.html",
    "destacado": true,
    "keywords": "manual instalacion sensor volumetrico sloc sdm bluetooth video configurar sensor conectar setup transmisión manual imán android"
  },
  {
    "id": "manual-instalacion-citisend",
    "titulo": "Manual de Instalación Citisend",
    "categoria": "Instalación",
    "icono": "🧰",
    "tipo": "HTML",
    "version": "v1.0",
    ...
    "tiempo": "8 min",
    "autor": "Operaciones",
    "descripcion": "Procedimiento operativo para la instalación de dispositivos Citisend en campo.",
    "archivo": "manuales/manual-instalacion-citisend.html",
    "destacado": true,
    "keywords": "manual instalacion citisend dispositivos campo instalador procedimiento montaje validacion puesta en marcha"
  },
  {
    "id": "manual-mantenimiento-preventivo",
    "titulo": "Manual de Mantenimiento Preventivo",
    ...
    "tiempo": "7 min",
    "autor": "Operaciones",
    "descripcion": "Guía para realizar revisiones preventivas, comprobaciones y validaciones periódicas.",
    "archivo": "manuales/manual-mantenimiento-preventivo.html",
    "destacado": true,
    "keywords": "manual mantenimiento preventivo citisend revision periodica comprobaciones validacion estado dispositivos"
  },
  {
    "id": "manual-mantenimiento-correctivo",
    "titulo": "Manual de Mantenimiento Correctivo",
    "categoria": "Mantenimiento",
    "icono": "🛠️",
    "tipo": "HTML",
    "version": "v1.0",
    ...
    "tiempo": "7 min",
    "autor": "Operaciones",
    "descripcion": "Guía para intervenir incidencias, diagnosticar fallos y registrar actuaciones correctivas.",
    "archivo": "manuales/manual-mantenimiento-correctivo.html",
    "destacado": true,
    "keywords": "manual mantenimiento correctivo citisend incidencia averia diagnostico reparacion actuacion correctiva"
  },
  {
    "id": "tarjetas-citisend",
    "titulo": "Manual Tarjetas Citisend",
    "categoria": "Tarjetas",
    "icono": "💳",
    "tipo": "HTML",
    "version": "v1.4",
    ...
    "tiempo": "6 min",
    "autor": "Operaciones",
    "descripcion": "Guía de operación para el uso, gestión y explicación de tarjetas ciudadanas.",
    "archivo": "manuales/tarjetas-citisend.html",
    "destacado": false,
    "keywords": "manual tarjetas ciudadanas citisend guia operacion usuarios tarjetas"
  },
  {
    "id": "generador-cae",
    "titulo": "CitiDocs · Generador CAE",
    "categoria": "CAE",
    "icono": "🦺",
    "tipo": "HTML",
    "version": "v1.4",
    ...
    "tiempo": "Herramienta",
    "autor": "Project Management",
    "descripcion": "Herramienta HTML para generar documentación CAE de forma guiada.",
    "archivo": "manuales/generador-cae.html",
    "destacado": true,
    "keywords": "generador cae citidocs coordinacion actividades empresariales subcontratas documento"
  }
];
...
async function loadJson(path, fallback){
  try{
    const response = await fetch(`${path}?v=2.0.1`, {cache:'no-store'});
...
function protectAdminHub(){
  const page = window.location.pathname.split('/').pop() || 'index.html';
  if(page === 'hub.html'){
    const role = sessionStorage.getItem('citisendRole');
    if(role !== 'admin'){
      window.location.replace(ACCESS_LOGIN_PAGE);
    }
  }
}

function isInstallerView(){
  const params = new URLSearchParams(window.location.search);
  return params.get('view') === 'instalador' || sessionStorage.getItem('citisendRole') === 'project';
}

function applyInstallerView(project){
  if(!isInstallerView()) return;
  const allowedProject = sessionStorage.getItem('citisendProject');
  if(allowedProject && allowedProject !== project.id){
    window.location.replace(`proyecto.html?id=${encodeURIComponent(allowedProject)}&view=instalador`);
    return;
  }
  document.body.classList.add('installer-view');
  document.querySelectorAll('.sidebar, .profile, #prevProject, #nextProject, #allProjectManuals, #adminNote').forEach(el => { if(el) el.style.display = 'none'; });
  const back = document.querySelector('.back-to-hub');
  if(back){
    back.href = ACCESS_LOGIN_PAGE;
    back.textContent = '← Salir del proyecto';
  }
  const library = document.getElementById('libraryLink');
  if(library) library.style.display = 'none';
  const gallery = document.getElementById('galleryLink');
  if(gallery) gallery.textContent = '📷 Ver galería';
  const jumpLabel = document.querySelector('.project-switcher label span');
  if(jumpLabel) jumpLabel.textContent = 'Proyecto asignado';
        ...
        <article class="project-card-premium">
          ...
          <div class="project-card-body">
            ...
            <div class="project-mini-kpis">
              ...
              <div><strong>${DEFAULT_PROJECT_MANUALS.length}</strong><small>Manuales</small></div>
            </div>
...
function renderProjectPage(){
  ...
  if(!project) return;
  applyInstallerView(project);

  document.title = `Citisend HUB · ${project.nombre}`;
  const percent = project.dispositivos ? Math.round((project.instalados / project.dispositivos) * 100) : 0;
  const projectManualIds = DEFAULT_PROJECT_MANUALS;
  ...
  setText('breadcrumbProject', project.nombre);
  setText('kpiClient', project.cliente || 'Cliente');
  ...
  setText('kpiPercentSmall', `${percent}%`);

  const planner = document.getElementById('plannerLink');
  if(planner) planner.href = project.planner || '#';
  const library = document.getElementById('libraryLink');
  if(library) library.href = `hub.html?proyecto=${encodeURIComponent(project.nombre)}#biblioteca`;
  const gallery = document.getElementById('galleryLink');
...
function setupProjectSwitcher(project){
  ...
  if(select){
    ...
    if(select.dataset.bound !== 'true'){
      select.addEventListener('change', () => {
        if(select.value) window.location.href = `proyecto.html?id=${encodeURIComponent(select.value)}${isInstallerView() ? '&view=instalador' : ''}`;
      });
      select.dataset.bound = 'true';
    }
  }
  ...
  if(prev && projects.length){
    ...
    prev.href = `proyecto.html?id=${encodeURIComponent(target.id)}${isInstallerView() ? '&view=instalador' : ''}`;
    prev.textContent = `← ${target.nombre}`;
  }
  if(next && projects.length){
    const target = projects[(index + 1) % projects.length];
    next.href = `proyecto.html?id=${encodeURIComponent(target.id)}${isInstallerView() ? '&view=instalador' : ''}`;
...
function renderRelatedManuals(relatedManuals, project){
  ...
  if(all) all.href = `hub.html?proyecto=${encodeURIComponent(project.nombre)}#biblioteca`;
}
